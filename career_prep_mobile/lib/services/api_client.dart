...
class ApiClient {
  ...
  // Change to your machine's LAN IP (e.g. http://192.168.1.x:5000/api) for a real device.
  ...
  Future<ResumeFeedback> analyzeResume(String resumeId) async {
    try {
      final response = await _dio.post<Map<String, dynamic>>('/resumes/$resumeId/analyze');
      return ResumeFeedback.fromJson(
          response.data?['feedback'] as Map<String, dynamic>? ?? <String, dynamic>{});
    } on DioException catch (exception) {
      _throwFriendly(exception, 'Could not analyze resume.');
    }
  }
